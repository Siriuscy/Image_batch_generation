...
// 导出所有画布为图片
document.getElementById('exportAllImages').addEventListener('click', async function() {
    try {
        // 让用户选择保存目录
        ...
        const scaleFactor = 2; // 设置导出图片的缩放因子，2表示2倍清晰度

        // 保存当前画布索引
        const currentIndex = currentCanvasIndex;
        ...
        for (let i = 0; i < generatedCanvases.length; i++) {
            ...
            // 生成文件名
            const fileName = `canvas_${i + 1}.jpg`;
...
document.getElementById('download-excel').addEventListener('click', function() {
    ...
    // 替换 XLSX.writeFile 这行代码
    const excelBuffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });

    dialog.showSaveDialog({
        title: '保存 Excel 文件',
        defaultPath: 'canvas_objects.xlsx',
        filters: [
